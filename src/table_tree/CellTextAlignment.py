...
class ColumnSortDemo(QWidget):
    ...
    def initUI(self):
        ...
        self.tableWidget.setHorizontalHeaderLabels(['姓名', '性别', '体重(kg)','hj'])
    ...
    def generateMemu(self, pos):
        ...
        for i in self.tableWidget.selectionModel().selection().indexes():
            rowNum = i.row()
            column=i.column()
            if self.tableWidget.item(rowNum,column):
                self.tableWidget.item(rowNum,column).setBackground(QBrush(QColor(155,155,255)))
                # self.tableWidget.item(2,3).setBackground(QBrush(QColor(155,155,255)))
            print(self.tableWidget.backgroundRole())
        if rowNum < 4:
            menu = QMenu()
